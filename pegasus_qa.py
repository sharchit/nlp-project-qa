...
def tokenize_function(tokenizer, examples):
    inputs = [f"question: {q} context: {c}" for c, q in zip(examples['context'], examples['question'])]
    ...
    with torch.no_grad():
        for batch in eval_loader:
            ...
            generated_ids = model.generate(
                ...
                num_beams=4,
                early_stopping=True,
                length_penalty=0.6  # Pegasus-specific optimization
            )
...
def run_pegasus_pipeline(model_id, data, output_file='pegasus_results.json'):
    ...
    training_args = Seq2SeqTrainingArguments(
        ...
        gradient_accumulation_steps=4  # Helps with memory
    )
...
if __name__ == "__main__":
    ...
    results = run_pegasus_pipeline("google/pegasus-xsum", data)
